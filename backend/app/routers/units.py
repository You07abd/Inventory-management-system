...
def list_units(item_id: int, db: Session = Depends(get_db)):
...
def create_unit(item_id: int, payload: UnitCreate, db: Session = Depends(get_db)):
    item = db.get(Item, item_id)
    if not item:
        raise HTTPException(status_code=404, detail="Item not found.")
...
def get_unit_by_asset_code(code: str, db: Session = Depends(get_db)):
    unit = (
        db.query(Unit)
...
def unit_cart_checkout(payload: UnitCartCheckoutRequest, db: Session = Depends(get_db)):
    user = db.get(User, payload.user_id)
    if not user:
        raise HTTPException(status_code=404, detail="User not found.")
